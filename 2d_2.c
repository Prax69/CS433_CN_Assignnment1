...
int process_packet(unsigned char *, int, int);
...
int main() {
    ...
    int inp;
    printf("Enter the desired portnumber, obtained by running 2d_1: ");
    scanf("%d",&inp);
    ...
    while (1) {
        ...
        if (process_packet(packet_buffer, packet_size,inp)) break;
    }
...
int process_packet(unsigned char *packet, int packet_size,int inp) {
    ...
    if (ntohs(tcp_header->th_sport) == inp) {
        printf("TCP Checksum: 0x%04x\n", ntohs(tcp_header->th_sum));
        printf("Found 'Flag' keyword in packet\n");
        printf("Source IP: %s\n", src_ip);
